...
void Visualizer::visualizeAvgRepulseVec(std::size_t state_num) {
  if (vis_data_.repulsed_vec_avg_at_link_.size() <= state_num ||
      vis_data_.repulsed_origin_at_link_.size() <= state_num) {
  ...
  std::vector<Eigen::Vector3d> repulsed_vec_at_link =
      vis_data_.repulsed_vec_avg_at_link_[state_num];
...
//       std::vector<double> joint_angles1 = utilities::toStlVec(vec_state1,
//       dof_);
...
//       std::vector<double> joint_angles2 = utilities::toStlVec(vec_state2,
//       dof_);
...
void Visualizer::visualizeRepulsedState() {
  ...
  while (user_input != "q") {
    ...
    visualizeTwoStates(vis_data_.sample_joint_angles_[viz_state_idx_],
