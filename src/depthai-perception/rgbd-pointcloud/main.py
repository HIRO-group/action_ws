...
def convertCloudFromOpen3dToRos(open3d_cloud, frame_id="map"):
    ...
    if not open3d_cloud.colors:  # XYZ only
        ...
        colors = colors[:, 2] * BIT_MOVE_16 + \
            colors[:, 1] * BIT_MOVE_8 + colors[:, 1]
...
plc_pub = rospy.Publisher("depthai_point_cloud", PointCloud2, queue_size=1)
        ...
        for q in qs:
            ...
            if new_msg is not None:
                ...
                if msgs:
                    ...
                    o3d_point_cloud = pcl_converter.rgbd_to_projection(
                        depth, rgb)
                    ros_point_cloud = convertCloudFromOpen3dToRos(
                        o3d_point_cloud)
                    plc_pub.publish(ros_point_cloud)
